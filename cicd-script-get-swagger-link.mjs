...
const loadOrFetchSwaggerFile = async (
	...
	if (!isFound) {
		...
		const dataSwagger = await downloadGithubFile(
			...
			githubUser,
			githubRepo,
			{ path: githubPath, branch: githubBranch },
		);
		if (!dataSwagger) {
			throw new Error("swagger.json not found in github.");
		}
	...
	return [swaggerKey, filePath, isFound ? true : false];
};

const pushCodeToRemote = async ({ fileArr, remoteBranch, allowPrefixArr }) => {
	const isAllow = allowPrefixArr.find((prefix) => remoteBranch.includes(prefix));
	if (!isAllow) {
	...
	if (!newFile2Push || newFile2Push.length === 0) {
...
const REMOTE_PUSH_BRANCH = process.env.REMOTE_PUSH_BRANCH || argv.REMOTE_PUSH_BRANCH;

const REMOTE_PUSH_BRANCH_ALLOW_PREFIX_ARR =
	process.env.REMOTE_PUSH_BRANCH_ALLOW_PREFIX_ARR ||
	argv.REMOTE_PUSH_BRANCH_ALLOW_PREFIX_ARR ||
	["uat/", "prd/"];

const main = async () => {
	const { swagger } = JSON.parse(await getFileContent({ FILE_URL, FILE_PATH }));
	if (!swagger) {
		throw new Error("Cannot find key: swagger");
	}
	...
	// for build script
	const azurePipelineScript = fileArr.map(([key, val]) => `"${key}=${val}"`).join(
		" ",
	);
